...
REM ---------------------------------------------------------------
REM  MeriWatch updater - just double click it.
REM
...
if not exist run.bat (echo [NG] Put this file in the MeriWatch folder and run it there. & echo      Current folder: %CD% & echo. & pause & exit /b 1)

REM Stop only the process holding port 5001, which is MeriWatch.
...
if errorlevel 1 (echo. & echo [NG] Download failed. Check your internet connection. & del /q app.py.new 2>nul & echo. & pause & exit /b 1)

REM A truncated download must never replace a working file.
set SIZE=0
for %%A in (app.py.new) do set SIZE=%%~zA
if %SIZE% LSS 50000 (echo. & echo [NG] Downloaded file is too small: %SIZE% bytes. Aborted. & del /q app.py.new 2>nul & echo. & pause & exit /b 1)
...
echo      Previous version kept as app.py.bak
echo.
